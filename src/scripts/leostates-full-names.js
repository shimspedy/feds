    ...
    stateElements.forEach(element => {
        ...
        if (stateMap[abbr]) {
            ...
            if (isLeoPage) {
                ...
                fetch('/data/leo-data.json')
                    ...
                    .then(data => {
                        ...
                        if (stateData && stateData[grade]) {
                            const pageTitle = `${grade} Law Enforcement Officer Salaries in ${fullName}`;
                            document.title = pageTitle;
                            if (element.tagName === 'H1') {
                                element.textContent = pageTitle;
                            }
                            updateMetaDescription(`${pageTitle} - Detailed information on salaries, hourly rates, and overtime rates for Law Enforcement Officer.`);
                        } else {
                            const pageTitle = `Law Enforcement Officer Salaries in ${fullName}`;
                            document.title = pageTitle;
                            if (element.tagName === 'H1') {
                                element.textContent = pageTitle;
                            }
                ...
                updateMetaDescription(`${pageTitle} - Explore detailed salary information for Law Enforcement Officer by state.`);
            }
        }
    });
}
